...
> **冲突处理（必读）**：**`docs/README.md`**「冲突与权威」。**MM2 已落盘**的消息与文件分片 **不会因进程崩溃/重启而按旧文档「丢弃」**。
...
3. 已 StoreMessage / StoreFileChunk 的数据仍在磁盘，可继续 Retrieve / GetFileChunk
...
| **IM 消息密文**（`StoreMessage`） | **保留**（`.zdb` + `im_messages`） |
...
- 无备份文件（产品策略）
- 无恢复接口（产品策略）
- **Level 2**：针对 **内存与策略性覆写**；**MM2 删除消息** 为零清 + 删索引行（见 **02-MessageRecall**、**03 第七节**）
